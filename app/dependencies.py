...
def _raise_unauthorized(*, clear_cookie: bool, response: Response) -> None:
    """Raise 401. Only mutate Set-Cookie when clearing a stale session (avoids 500 on missing cookie)."""
    if clear_cookie:
...
async def require_user(
    ...
    if not sa_session:
        _raise_unauthorized(clear_cookie=False, response=response)

    user_id = store.get_user_id_from_session(db, sa_session)
    if not user_id:
        _raise_unauthorized(clear_cookie=True, response=response)
